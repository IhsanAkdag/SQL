...
SELECT p1.isim AS personel_adi,p2.isim AS yonetici_adi
...
SELECT * FROM musteriler WHERE isim ILIKE 'A%' --kucuk harflerde geir
SELECT * FROM musteriler WHERE isim LIKE 'A%'
SELECT * FROM musteriler WHERE isim ~~ 'A%' -- like yerine ~~ kullanabiliriz
SELECT * FROM musteriler WHERE isim ~~* 'A%' --kucuk harfleride getirir
...
SELECT * FROM kelimeler WHERE kelime ~* 'h[ai]t' --  "*" yazarsak buyuk kucuk harf gozetmez

-- SORU	: Ilk harfi h,son harfi t olup 2.harfi a ile k arasinda olan 3 harfli kelimelerin  tum bilgilerini  yazdiran 
-- QUERY yazin
SELECT * FROM kelimeler WHERE kelime ~* 'h[a-k]t' --  "-" iki harf arasindaki harfler

-- SORU	: a veya s ile baslayan kelimelerin tum bilgilerini yazdiran QUERY yazin
SELECT * FROM kelimeler WHERE kelime ~* '^[as]'  --   "^" baslayan demek

-- SORU	: m veya f ile biten kelimelerin tum bilgilerini yazdiran QUERY yazin
SELECT * FROM kelimeler WHERE kelime ~* '[mf]$' -- "$" biten demek
