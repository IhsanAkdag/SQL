...
--Insanlar tablosundaki ismi Mine olanlari SSN (kucukten ten buyuge) sirali olarak listeleyin
...
SELECT * FROM insanlar ORDER BY ssn DESC; --ters sirlama
...
SELECT isim,sum(urun_miktar) AS aldığı_toplam_urun FROM manav GROUP BY isim ORDER BY aldığı_toplam_urun;
